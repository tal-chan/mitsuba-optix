...
extern "C" {
  ...
  * <B>Description</B>
  * 
  * An OptiX buffer in an unregistered state can be registered to OptiX again via @ref rtBufferGLRegister. Once registered, properties like the 
  * size of the original GL resource cannot be modified anymore. Calls to the corresponding GL functions will return with an error code. 
  * However, the data of the GL resource can still be read and written by the appropriate GL commands. When a buffer is already in a registered 
  * state @ref rtBufferGLRegister will return @ref RT_ERROR_RESOURCE_ALREADY_REGISTERED. A resource must be registered in order to be used by OptiX. 
  * If a resource is not registered @ref RT_ERROR_INVALID_VALUE will be returned. 
  *
  ...
  * <B>Description</B>
  * 
  * An OptiX buffer in a registered state can be unregistered via @ref rtBufferGLRegister. Once unregistered, properties like the size of the original GL 
  * resource can be changed. As long as a resource is unregistered, OptiX will not be able to access the data and will fail with @ref RT_ERROR_INVALID_VALUE. When a buffer is already 
  * in an unregistered state @ref rtBufferGLUnregister will return @ref RT_ERROR_RESOURCE_NOT_REGISTERED.
  *
  * @param[in]   buffer          The handle for the buffer object
  * 
  * <B>Return values</B>
  ...
  * <B>Description</B>
  * 
  * An OptiX texture sampler in an unregistered state can be registered to OptiX again via @ref rtTextureSamplerGLRegister. Once registered, properties like the size 
  * of the original GL resource cannot be modified anymore. Calls to the corresponding GL functions will return with an error code. However, the data of the GL resource 
  * can still be read and written by the appropriate GL commands. When a texture sampler is already in a registered state @ref rtTextureSamplerGLRegister will return @ref RT_ERROR_RESOURCE_ALREADY_REGISTERED. 
  ...
  * <B>Description</B>
  * 
  * An OptiX texture sampler in a registered state can be unregistered via @ref rtTextureSamplerGLUnregister. Once unregistered, properties like the size of the original GL 
  * resource can be changed. As long as a resource is unregistered, OptiX will not be able to access the data and will fail with @ref RT_ERROR_INVALID_VALUE. When a buffer 
  * is already in an unregistered state @ref rtBufferGLUnregister will return \a T_ERROR_RESOURCE_NOT_REGISTERED.
  *
  * @param[in]   textureSampler          The handle for the texture object
  * 
  * <B>Return values</B>
