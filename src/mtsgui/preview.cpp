...
PreviewThread::PreviewThread(Device *parentDevice, Renderer *parentRenderer)
	...
	MTS_AUTORELEASE_END()
	//initializing path tracer
	unsigned int sqrt_num_samples = 2u;
	unsigned int width = 512u, height = 512u;
	m_path_tracer_scene.setNumSamples(sqrt_num_samples);
	m_path_tracer_scene.setDimensions(width, height);
	m_path_tracer_scene.setUseVBOBuffer(true);
	m_sceneChanged = true;
}
...
void PreviewThread::run() {
	...
	try {
		...
		//Initializing path tracer scene

		m_path_tracer_scene.enableCPURendering(false);
		m_path_tracer_scene.setNumDevices(1);
		SampleScene::InitialCameraData pt_camera_data;
		m_path_tracer_scene.initScene(pt_camera_data);
		//m_path_tracer_scene.validateAndCompile();
		...
		while (true) {
			...
			if (method != EOpenGL) {
				if (method == EOptiX){
					...
					if (m_sceneChanged){
						std::vector<Renderer::TransformedGPUGeometry>* geometry = m_shaderManager->getGeometry();
						for (unsigned int i = 0; i < geometry->size(); i++){
							Renderer::TransformedGPUGeometry trans_geo = (*geometry)[i];
							Matrix4x4 transfo = trans_geo.second;
							float trans[16];
							for (int k = 0; k < 4; k++){
								for (int l = 0; l < 4; l++){
									trans[k * 4 + l] = transfo(k,l);
								}
							}
							const GLGeometry * geo = static_cast<const GLGeometry*> (trans_geo.first);
							unsigned int id[2];
							id[0] = geo->m_id[GLGeometry::EVertexID];
							id[1] = geo->m_id[GLGeometry::EIndexID];
							Log(EDebug, "buffer id : %d\n");
							unsigned int triCount = geo->getTriMesh()->getTriangleCount();
							unsigned int vertexCount = geo->getTriMesh()->getVertexCount();
							bool hasNormals = geo->getTriMesh()->hasVertexNormals();
							bool hasTexCoord = geo->getTriMesh()->hasVertexTexcoords();
							bool hasUVTangents = geo->getTriMesh()-> hasUVTangents();
							bool hasColors = geo->getTriMesh()->hasVertexColors();
							m_path_tracer_scene.addTransformedShape(id, triCount, vertexCount, hasNormals, hasTexCoord, hasUVTangents, hasColors, trans);

						}
						m_path_tracer_scene.finishGeometry();
						m_path_tracer_scene.validateAndCompile();
						m_sceneChanged = false;
					}
					optix::Buffer pt_buffer = m_path_tracer_scene.getOutputBuffer();
					RTsize pt_buffer_width_rts, pt_buffer_height_rts;
					pt_buffer->getSize(pt_buffer_width_rts, pt_buffer_height_rts);
					int pt_buffer_width = static_cast<int>(pt_buffer_width_rts);
					int pt_buffer_height = static_cast<int>(pt_buffer_height_rts);

					if (pt_buffer_width != size[0] || pt_buffer_height != size[1]){
						//if needed, resize path tracer scene
						m_path_tracer_scene.resize(size[0], size[1]);
					}
					renderOptix(target);
...
void PreviewThread::renderOptix(PreviewQueueEntry &target){
	if (!m_context->scene->getSensor()->getClass()->derivesFrom(MTS_CLASS(ProjectiveCamera))) {
		/* This camera type is not supported! */
		target.buffer->activateTarget();
		target.buffer->clear();
	...
	Point p = worldTransform.transformAffine(Point(0.0f));
	Vector u = worldTransform(Vector(-1.0f,0.0f,0.0f));
	Vector v = worldTransform(Vector(0.0f, 1.0f, 0.0f));
	Vector w = worldTransform(Vector(0.0f, 0.0f, 1.0f));
	float u_len = 1/projTransform(Vector(1.0f, 0.0f, 0.0f)).x;
	float v_len = 1 / projTransform(Vector(0.0f, 1.0f, 0.0f)).y;
	u *= u_len;
	v *= v_len;
	optix::float3 eye = optix::make_float3(p.x, p.y, p.z);
	optix::float3 U = optix::make_float3(u.x, u.y, u.z); 
	optix::float3 V = optix::make_float3(v.x, v.y, v.z);
	optix::float3 W = optix::make_float3(w.x, w.y, w.z);
	//pt_camera.getEyeUVW(eye, U, V, W);
	SampleScene::RayGenCameraData pt_rg_camera_data(eye, U, V, W);
	m_path_tracer_scene.trace(pt_rg_camera_data);

	target.buffer->bind();
	m_path_tracer_scene.renderToActiveTexture();
	target.buffer->unbind();
}
